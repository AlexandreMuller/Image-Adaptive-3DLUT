...
class ImageDataset_HDRplus(Dataset):
    def __init__(self, root, mode="train"):
        ...
        train_names = _read_file_list(root / 'train.txt')
        test_names = _read_file_list(root / 'test.txt')

        self.train_inputs = [str(root / "middle_480p" / (n + ".png")) for n in train_names]
        self.train_experts = [str(root / "output_480p" / (n + ".jpg")) for n in train_names]
        self.test_inputs = [str(root / "middle_480p" / (n + ".png")) for n in test_names]
        self.test_experts = [str(root / "output_480p" / (n + ".jpg")) for n in test_names]
...
class ImageDataset_HDRplus_unpaired(Dataset):
    def __init__(self, root, mode="train"):
        ...
        self.set1_inputs = [str(root / "middle_480p" / (n + ".png")) for n in train_names]
        self.set1_experts = [str(root / "output_480p" / (n + ".jpg")) for n in train_names]
        self.set2_inputs = self.set1_inputs[:]
        self.set2_experts = self.set1_experts[:]
        self.test_inputs = [str(root / "middle_480p" / (n + ".png")) for n in test_names]
        self.test_experts = [str(root / "output_480p" / (n + ".jpg")) for n in test_names]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            W2, H2 = img2.size
            ratio = np.random.uniform(0.6, 1.0)
            crop_h = min(round(H * ratio), H2)
            crop_w = min(round(W * ratio), W2)
            i, j, h, w = transforms.RandomCrop.get_params(img_exptC, output_size=(crop_h, crop_w))
            img_input = TF_x.crop(img_input, i, j, h, w)
            img_exptC = TF.crop(img_exptC, i, j, h, w)
            i, j, h, w = transforms.RandomCrop.get_params(img2, output_size=(crop_h, crop_w))
            ...
            img_input = TF_x.adjust_brightness(img_input, np.random.uniform(0.8, 1.2))
